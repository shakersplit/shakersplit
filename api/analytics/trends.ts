...
/**
 * Trends endpoint — returns daily-bucketed counts for the last N days, suitable for a
 * line/bar chart on the Analytics page. Default range = 30 days, max 90.
 *
 * Response shape:
 *   { days: [{ date: 'YYYY-MM-DD', food_meals, food_calories, workout_minutes, alcohol_drinks, weight_kg | null }, ...] }
 */
export default createHandler({
  async GET(req, res, user) {
    ...
    // Build a date string keyed map. UTC dates so the buckets align with how the DB stores rows.
    ...
    // Pre-fill every day in range so the chart has a continuous x-axis, even on quiet days.
    ...
    (weightRows.data ?? []).forEach((r) => {
      const b = ensure(dateKey(r.logged_at));
      // Latest weight wins if multiple in a day — sort by date desc isn't guaranteed here.
      // Take the simple average instead so we don't lose information.

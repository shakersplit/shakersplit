 ...
 *   - today's totals (food meals + calories, workout count + minutes, alcohol drinks)
 *   - weekly totals (last 7 days)
 *   - current streaks for the user
 *   - latest weight entry
 *
 * One query per signal so we can hit the user's own indexes; everything is parallelized.
 */
export default createHandler({
  async GET(_req, res, user) {
    ...
    const [
      ...
      supabaseAdmin
        .from('food_logs')
        .select('total_calories', { count: 'exact' })
        .eq('user_id', user.id)
        .gte('logged_at', todayISO),
      supabaseAdmin
        .from('food_logs')
        .select('total_calories, logged_at', { count: 'exact' })
        .eq('user_id', user.id)
        .gte('logged_at', weekStartISO),
      supabaseAdmin
        .from('workout_logs')
        .select('duration_minutes, calories_burned', { count: 'exact' })
        .eq('user_id', user.id)
        .gte('logged_at', todayISO),
      supabaseAdmin
        .from('workout_logs')
        .select('duration_minutes, calories_burned, logged_at', { count: 'exact' })
        .eq('user_id', user.id)
        .gte('logged_at', weekStartISO),
      supabaseAdmin
        .from('alcohol_logs')
        .select('quantity_ml', { count: 'exact' })
        .eq('user_id', user.id)
        .gte('logged_at', todayISO),
      supabaseAdmin
        .from('alcohol_logs')
        .select('quantity_ml, logged_at', { count: 'exact' })
        .eq('user_id', user.id)
        .gte('logged_at', weekStartISO),
      supabaseAdmin
        .from('activity_streaks')
        .select('streak_type, current_count, longest_count, last_logged_at')
        .eq('user_id', user.id),
      supabaseAdmin
        .from('weight_logs')
    ...
    const errs = [foodToday, foodWeek, workoutToday, workoutWeek, alcoholToday, alcoholWeek, streaks, latestWeight]
      .map((r) => r.error)
      .filter(Boolean);
    if (errs.length > 0) {
      return error(res, 500, 'INTERNAL_ERROR', errs[0]?.message ?? 'Aggregation failed');
    }
    ...
    return success(res, {
      today: {
        ...
        workout: {
          sessions: workoutToday.count ?? 0,
          minutes: sumMinutes(workoutToday.data as { duration_minutes: number | null }[] | null),
        },
      ...
      week: {
        ...
        workout: {
          sessions: workoutWeek.count ?? 0,
          minutes: sumMinutes(workoutWeek.data as { duration_minutes: number | null }[] | null),
        },
      ...
      latest_weight: latestWeight.data
        ? {
            weight_kg: latestWeight.data.weight_kg,
            body_fat_pct: latestWeight.data.body_fat_pct,
            logged_at: latestWeight.data.logged_at,
          }
        : null,
    });
  },
